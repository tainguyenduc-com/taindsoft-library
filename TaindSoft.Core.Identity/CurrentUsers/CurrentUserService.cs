...
namespace TaindSoft.Core.Identity.CurrentUsers
{
        ...
        ILogger<CurrentUserService> logger) : ICurrentUserService
    {
        ...
        {
            // If external client, validate against permission claims
            if (IsExternalClient())
            {
                var scopes = GetExternalScopes();
                if (!scopes.Contains(permissionCode))
                {
                    throw new UnauthorizedException("External client does not have the required permission.");
                }
                return Task.CompletedTask;
            }

            // Existing behavior for traditional users (simplified for this codebase)
        ...
        public bool IsExternalClient()
        {
            ...
            return user.HasClaim(c => c.Type == "client_id");
        }
        ...
        public IEnumerable<string> GetExternalScopes()
        {
            ClaimsPrincipal? user = httpContextAccessor.HttpContext?.User;
